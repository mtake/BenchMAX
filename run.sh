...
case "$TASK" in
    ...
    "rule_based"|"xifeval")
        if [ "$LANG" == "all" ]; then
            ...
            lm-eval -m vllm --model_args pretrained=${MODEL} --tasks $tasks --batch_size auto --apply_chat_template --include_path tasks/ifeval --log_samples -o results
        fi
        ;;
    ...
    "math"|"xmgsm")
        if [ "$LANG" == "all" ]; then
            ...
            lm-eval -m vllm --model_args pretrained=${MODEL} --tasks $tasks --batch_size auto --apply_chat_template --include_path tasks/mgsm --log_samples -o results
        fi
        ;;
    ...
    "science"|"xgpqa")
        if [ "$LANG" == "all" ]; then
            ...
            lm-eval -m vllm --model_args pretrained=${MODEL} --tasks $tasks --batch_size auto --apply_chat_template --include_path tasks/gpqa --log_samples -o results
        fi
        ;;
    # --------------------------- Long Context Task ---------------------------
